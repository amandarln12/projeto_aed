...
// Suas estruturas de cartas e outras definições aqui...
typedef struct cartas CARTAS;
typedef struct ataque ATAQUE;
typedef struct defesa DEFESA;
typedef struct especial ESPECIAL;
typedef struct tipo_carta TIPO_CARTA;
typedef struct deck DECK;

typedef struct ataque
{
    int furia;//valor somado ao acumulador de critico
    int qtd_ataque; //quantas vezes o jogador ataca ao usar esta carta NAQUELE TURNO
...
{
    int revestimento; //quantos turnos o jogador ganha um escudo ao usar esta carta
    int qtd_defesa; //quantas vezes o jogador se defende ao usar esta carta NAQUELE TURNO
    int energia_devolvida; //quanto de energia é devolvida ao jogador
    int exilar; //0 para empilhar, 1 para não empilhar
}DEFESA;
...
typedef struct especial
{
    ...
    */
   int energia;
   /*O valor de energia define o quanto o player ganha de energia extra pra gastar naquele turno e o custo é 0*/
}ESPECIAL;
